...
$("document").ready( function() {
  ...
  $(".page-container.user-dashboard .btn.accept-offer").on('click', function() {
   if ($(this).closest(".portlet").find("input:checked").length == 0) {
     $(this).notify("None selected",{arrowShow: false});
     return false;
   }
   if ($(this).closest(".portlet").find("input:checked").length >1) {
     $(this).notify("Select 1",{arrowShow: false});
     return false;
   }
   if ($(this).closest(".portlet").find("input:checked").first().closest('tr').data("ntype") != "offer" ) {
     $(this).notify("Select an offer to accept it",{arrowShow: false});
     return false;
   }
   ...
   $.ajax({
     ...
     dataType: "json",
     url: '/negotiations',
       data: { negotiation: { src: src, dst: dst, details: null, ndate: null, ntype: "accept-offer" } },
     beforeSend:function(){
       //$(".page-container.tickets-show .timeline-body-content:last").css('background', "rgba(255,255,255,0.5)");
     },
     success:function(data){
       $(".page-container.user-dashboard .offers.refreshable").html("foo");
     },
     error:function(){
       //alert('Ajax error');
     }
  ...
  $(".page-container.user-dashboard .btn.counter-offer").on('click', function() {
   console.log("clicked");
  });
